...
async def test_reset_password_invalid_token(client, db_session, seed_user_groups):
    ...
    Validates that the endpoint returns a 400 status code and an appropriate
    error message when an invalid token is provided.
...
@pytest.mark.asyncio
async def test_change_password_success(client, user_token, db_session, seed_user_groups):
    """
    Test successful password change for an authenticated user.

    Steps:
    - Authenticate a user and obtain a valid access token.
    - Send a request to the change-password endpoint with correct current credentials.
    - Verify the response status code and success message.
    - Fetch the user from the database.
    - Verify that the stored password hash matches the new password.

    Expected result:
    - 200 OK response.
    - Response message confirms successful password change.
    - User password is updated in the database.
    """

    email = user_token["email"]
    password = user_token["raw_password"]
    ...
    assert reset_response.status_code == 200, "Expected status code 200 for successful password change."
    assert reset_response.json()["message"] == "Password changed successfully.", (
        "Unexpected response message for password change."
    )
    ...
    assert db_user.verify_password(new_password), "Password should be updated successfully in the database."
...
@pytest.mark.asyncio
async def test_logout_success(client, user_token, db_session, jwt_manager, seed_user_groups):
    """
    Test successful user logout.

    Steps:
    - Retrieve a valid refresh token for an authenticated user.
    - Verify the refresh token exists in the database before logout.
    - Send a logout request with the refresh token.
    - Verify the response status code and success message.
    - Check that the refresh token is removed from the database.

    Expected result:
    - 200 OK response.
    - Response message confirms successful logout.
    - Refresh token is deleted from the database.
    """

    user_refresh_token = user_token["refresh_token"]

    stmt = select(RefreshTokenModel).filter_by(token=user_refresh_token)
    result = await db_session.execute(stmt)
    refresh_token = result.scalars().first()
    assert refresh_token is not None, "Refresh token should exist before logout."

    logout_payload = {"refresh_token": user_refresh_token}
    logout_response = await client.post("/api/v1/accounts/logout/", json=logout_payload)

    assert logout_response.status_code == 200, "Expected status code 200 logout."
    assert logout_response.json()["message"] == "Logged out successfully.", "Unexpected error message."

    stmt = select(RefreshTokenModel).filter_by(token=user_refresh_token)
    result = await db_session.execute(stmt)
    refresh_token_record = result.scalars().first()

    assert refresh_token_record is None, "Refresh token should not exist in database."
