...
from cinema.database import UserGroupEnum
from cinema.database.models.accounts import UserModel, UserProfileModel, UserGroupModel
from cinema.exceptions import S3FileUploadError
...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_create_user_profile_expired_token(client, db_session, jwt_manager, seed_user_groups):
    """
...
@pytest.mark.unit
@pytest.mark.parametrize(
    "info_value, expected_msg",
    [
        ("", "Field required"),
        ("   ", "Info field cannot be empty or contain only spaces."),
    ],
)
async def test_profile_creation_empty_info(client, jwt_manager, info_value, expected_msg):
    """
    ...
    assert ("Info field cannot be empty or contain only spaces."
            in error_text or "Field required" in error_text), (
